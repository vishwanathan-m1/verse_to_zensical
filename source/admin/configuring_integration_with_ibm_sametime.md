...
-   Configure Sametime integration for {{ fullVerseProductName }}. For more information, see the topic [Integrating with {{ fullVerseProductName }}](https://pages.git.cwp.pnp-hcl.com/CWPdoc/sametime-MKDocs/v1203/admin/integrating-with-HCL-{{ shortVerseProductName }}.html) in Sametime documentation.

## Procedure

Add the following settings to the notes.ini file on the Domino server that hosts {{ shortVerseProductName }}, then restart the HTTP task:
...
## Enabling Sametime logout with {{ shortVerseProductName }} logout

Beginning with HCL Verse 3.2.2, user can be logged out of Sametime when logging out of {{ shortVerseProductName }}.
